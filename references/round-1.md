...
| Incremental value | What is added beyond an ordinary capable agent with comparable tools? Without matched evidence, describe the proposed advantage as unproven. |
| Philosophy and reasoning | Underlying thesis, values, empirical assumptions, heuristics, causal logic, scope, counterexamples and willingness to revise |
| Design and architecture | Inputs, tools, state, dependencies, boundaries, modularity, coupling, testability, validation, recovery and justified design tradeoffs |
...
| Authority, security and privacy | Read/write power, credential handling, outbound content, telemetry defaults, optional versus mandatory channels, enforcement outside the model |
| Evidence and validation | Reproducible fixtures, actual results, source provenance, held-out cases, limitations; separate package checks from behavioral tests |
| Efficiency and maintainability | Runtime portability, install/update behavior, model/API cost, latency, attention demanded, maintenance burden, license and distribution rights |
...
3. Review [design and doctrine](design-and-doctrine.md): reconstruct the thesis fairly, challenge assumptions, map principles to mechanisms, and evaluate architecture plus alternatives. Compare advertised behavior with code. Check success signaling, fallback paths, and how the package handles inadequate evidence.
...
5. Produce findings, a [category scorecard](scoring.md) at standard/deep scope or when requested, and a readiness assessment. Distinguish required fixes from optional improvements. State what the review could not establish.
